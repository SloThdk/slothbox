...
| Subpoena / legal compulsion                              | The operator can hand over ciphertext only — the keys never reach the server. Connection metadata exists transiently (client IP in edge access logs for 7 days and in rate-limit counters for up to 24 h; timestamps and share IDs in the audit chain), and that scope is documented on the live /transparency page |
